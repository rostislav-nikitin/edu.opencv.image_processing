...
public:
	...
	cv::Mat getHueHistogram(cv::Mat const &image, int minSat = 0)
	{
		cv::Mat hist;
...
int main()
{
	...
	cv::Rect drawRect(cv::Point(377, 189), cv::Point(434, 235));
	//cv::rectangle(image, drawRect, cv::Scalar(0, 0, 255));

	cv::namedWindow("Original");
	cv::imshow("Original", image);

	cv::Rect rect(cv::Point(378, 190), cv::Point(433, 234));
	...
	cv::Mat hist = histogram.getHueHistogram(imageROI, 77);
	...
	cv::Mat result = cf.find(hueChannel);

	cv::namedWindow("Probability map");
	//cv::imshow("Probability map", histogram.invert(result));
	cv::imshow("Probability map", result);


	cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 10, 1);
	cv::Rect startRect(cv::Point(377+50, 189+50), cv::Point(434+50, 235+50));
	cv::rectangle(image, startRect, cv::Scalar(0, 0, 255));

	std::cout << "Iter count=" << cv::meanShift(result, startRect, criteria) << std::endl;
	cv::rectangle(image, startRect, cv::Scalar(0, 255, 0));

	cv::namedWindow("OriginalwRect");
	cv::imshow("OriginalwRect", image);
