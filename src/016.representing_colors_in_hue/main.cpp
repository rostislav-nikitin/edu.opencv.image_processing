...
int main()
{
	...
	cv::imshow("Saturation", hue[1]);
	cv::namedWindow("Brightness");
	cv::imshow("Brightness", hue[2]);

	hue = 255;
	cv::Mat newImage;
	//cv::Mat(hue[2].clone());
	cv::merge(hue, newImage);
	cv::cvtColor(newImage, newImage, cv::COLOR_HSV2BGR);
	cv::namedWindow("Photo fix");
	cv::imshow("Photo fix", newImage);
	...
	for(int h = 0; h < 360; h++)
	{
		for(int s = 0; s < 128; s++)
		{
			...
			hs.at<cv::Vec3b>(s, h)[2] = 128;
		}
	}

	cv::Mat hsAsBGR;
	cv::cvtColor(hs, hsAsBGR, cv::COLOR_HSV2BGR);
	cv::namedWindow("HS");
	cv::imshow("HS", hsAsBGR);
